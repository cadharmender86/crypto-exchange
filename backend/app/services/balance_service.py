...
class BalanceService:
    ...
    @staticmethod
    async def get_locked_account(db: AsyncSession, account_id: UUID) -> Account:
    ...
    @staticmethod
    async def consume_locked(account: Account, amount: Decimal) -> None:
        """Permanently consume funds already reserved for a pending order."""
        amount = BalanceService._amount(amount)
        BalanceService._validate_account(account)
        locked = Decimal(str(account.locked_balance))
        if locked < amount:
