...
type NavItem = { href: string; label: string; icon: string; permission: string; superAdminOnly?: boolean };
  ...
  return (
    <div className="min-h-screen bg-[#020b1c] text-slate-100 lg:flex">
      <aside className="hidden h-screen w-[278px] shrink-0 border-r border-slate-800/80 bg-[#031027] lg:sticky lg:top-0 lg:flex lg:flex-col">
        <div className="flex h-[72px] shrink-0 items-center gap-3 border-b border-slate-800/80 px-7"><div className="text-[37px] font-black leading-none text-blue-600">B</div><div className="text-[22px] font-bold tracking-tight text-slate-100">BitNova</div></div>
        <nav className="min-h-0 flex-1 overflow-y-auto px-4 py-5">
          {visibleGroups.map((group) => { const active = isGroupActive(group); const collapsed = Boolean(collapsedSections[group.label]) && !active; return <div key={group.label} className="mb-3 last:mb-1">
            <button type="button" onClick={() => toggleSection(group.label)} className={`flex w-full items-center justify-between rounded-lg px-3 py-2.5 text-left text-[14px] font-extrabold tracking-wide transition ${active ? "text-blue-300" : "text-slate-200 hover:bg-slate-800/40 hover:text-white"}`} aria-expanded={!collapsed} aria-controls={`admin-nav-${group.label.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`}>
              <span>{group.label}</span><span className={`text-[14px] transition-transform ${collapsed ? "-rotate-90" : "rotate-0"}`}>⌄</span>
            </button>
            {!collapsed && <div id={`admin-nav-${group.label.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`} className="mt-1 space-y-1">{group.items.map((item) => <Link key={`${item.label}-${item.href}`} href={item.href} className={`flex items-center gap-3 rounded-lg px-3 py-2.5 text-[13px] font-medium transition ${isActive(item.href) ? "bg-blue-600 text-white shadow-lg shadow-blue-950/30" : "text-slate-300 hover:bg-slate-800/50 hover:text-white"}`}><span className="grid w-5 place-items-center text-[16px] opacity-90">{item.icon}</span><span className="truncate">{item.label}</span></Link>)}</div>}
          </div>; })}
        </nav>
        <div className="shrink-0 border-t border-slate-800/80 p-4"><div className="flex items-center gap-3 px-1 py-2"><div className="relative"><div className="grid h-10 w-10 place-items-center rounded-full bg-indigo-500 text-sm font-semibold text-white">{initials}</div><span className="absolute bottom-0 right-0 h-2.5 w-2.5 rounded-full border-2 border-[#031027] bg-emerald-400" /></div><div className="min-w-0 flex-1"><div className="truncate text-sm font-semibold text-white">{adminLabel}</div><div className="truncate text-[11px] text-slate-400">{session.email}</div></div><button onClick={logout} aria-label="Logout" className="text-slate-400 hover:text-white">⌄</button></div></div>
      </aside>
      <main className="min-w-0 flex-1">
        <header className="sticky top-0 z-20 flex h-[72px] items-center justify-between border-b border-slate-800/80 bg-[#020b1c]/95 px-4 backdrop-blur lg:px-7"><div className="flex min-w-0 items-center gap-4"><button className="grid h-9 w-9 place-items-center rounded-lg border border-slate-800 bg-[#07142a] text-slate-300 lg:hidden">☰</button><div className="hidden text-sm text-slate-400 sm:block">Admin / <span className="text-slate-200">{flatNav.find((item) => isActive(item.href))?.label || "Dashboard"}</span></div><div className="relative hidden md:block"><span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-500">⌕</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search anything..." className="h-10 w-72 rounded-full border border-slate-700 bg-[#07142a] pl-9 pr-4 text-xs text-slate-200 outline-none placeholder:text-slate-500 focus:border-blue-500/70" /></div></div><div className="flex items-center gap-3"><button className="relative grid h-9 w-9 place-items-center text-xl text-slate-300" aria-label="Notifications">♧<span className="absolute right-0.5 top-0.5 h-2 w-2 rounded-full bg-rose-500" /></button><button className="grid h-9 w-9 place-items-center text-lg text-slate-300" aria-label="Theme">◐</button><button onClick={logout} className="grid h-9 w-9 place-items-center text-lg text-slate-300" aria-label="Logout">↪</button></div></header>
