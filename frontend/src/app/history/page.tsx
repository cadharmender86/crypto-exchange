          ...
          <div className="flex flex-col gap-4 border-b border-white/10 p-5 xl:flex-row xl:items-center xl:justify-between">
            <div className="flex flex-wrap gap-1 rounded-lg border border-white/10 bg-[#080d12] p-1">
              {(["ALL", "DEPOSIT", "WITHDRAWAL", "TRADE", "TRANSFER"] as Filter[]).map((item) => (
                <button
                  ...
                  className={`rounded-md px-3 py-2 text-xs font-medium transition ${filter === item ? "bg-blue-600 text-white" : "text-slate-400 hover:text-white"}`}
                >
            ...
            <div className="flex flex-col gap-2 sm:flex-row">
              <input
                ...
                className="w-full rounded-lg border border-white/10 bg-[#080d12] px-4 py-2.5 text-sm outline-none placeholder:text-slate-600 focus:border-blue-500 sm:w-64"
              />
              <select
                value={asset}
                onChange={(event) => changeAsset(event.target.value)}
                className="rounded-lg border border-white/10 bg-[#080d12] px-4 py-2.5 text-sm text-slate-300 outline-none focus:border-blue-500"
              >
            ...
            <table className="w-full min-w-[1050px] text-left text-sm">
              ...
              <tbody>
                    ...
                    <tr key={`${item.id}-${item.reference}-${index}`} className="border-b border-white/5 hover:bg-white/[0.025]">
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          <CoinIcon symbol={itemAsset} size={34} />
                          <span className="font-semibold">{itemAsset}</span>
                        </div>
                      </td>
                      <td className={`px-5 py-4 font-semibold ${direction === "CREDIT" ? "text-emerald-400" : "text-red-400"}`}>
                        {formatAmount(item.amount ?? 0, itemAsset, direction)}
                      </td>
                      <td className="px-5 py-4">
                        <span className="rounded-full border border-emerald-500/20 bg-emerald-500/10 px-2.5 py-1 text-[11px] font-semibold text-emerald-400">{status || "UNKNOWN"}</span>
                      </td>
                ...
                {!loading && visible.length === 0 && (
                  <tr>
                    <td colSpan={6} className="px-5 py-16 text-center text-sm text-slate-500">No transactions match your filters.</td>
                  </tr>
                )}
              </tbody>
            </table>
