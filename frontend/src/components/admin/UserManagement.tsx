...
export default function UserManagement() {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      <div><h2 className="text-2xl font-bold tracking-tight">User management</h2><p className="mt-2 text-sm text-slate-500">Search, review and control customer accounts.</p></div>
      <div className="rounded-2xl border border-slate-800 bg-[#0d1422] p-5">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
          <input value={search} onChange={(e) => { setPage(1); setSearch(e.target.value); }} placeholder="Search by email..." className="w-full rounded-xl border border-slate-700 bg-slate-950/50 px-4 py-2.5 text-sm outline-none placeholder:text-slate-600 focus:border-cyan-500 lg:max-w-md" />
        ...
        <div className="mt-5 overflow-x-auto">
          <table className="w-full min-w-[900px] text-left text-sm"><thead className="border-b border-slate-800 text-xs uppercase tracking-wider text-slate-500"><tr><th className="pb-3 pr-4">User</th><th className="pb-3 pr-4">Account</th><th className="pb-3 pr-4">KYC</th><th className="pb-3 pr-4">2FA</th><th className="pb-3 pr-4">Last login</th><th className="pb-3 text-right">Action</th></tr></thead>
            <tbody>{loading ? <tr><td colSpan={6} className="py-12 text-center text-slate-500">Loading users...</td></tr> : users.length === 0 ? <tr><td colSpan={6} className="py-12 text-center text-slate-500">No users found.</td></tr> : users.map((user) => <tr key={user.id} className="border-b border-slate-800/70 last:border-0"><td className="py-4 pr-4"><div className="font-semibold text-slate-200">{user.email}</div><div className="mt-1 text-xs text-slate-600">{user.id}</div></td><td className="py-4 pr-4"><span className={user.is_active ? "rounded-full bg-emerald-500/10 px-2.5 py-1 text-xs text-emerald-300" : "rounded-full bg-red-500/10 px-2.5 py-1 text-xs text-red-300"}>{user.is_active ? "Active" : "Suspended"}</span></td><td className="py-4 pr-4 text-slate-400">{user.is_verified ? "Verified" : "Unverified"}</td><td className="py-4 pr-4 text-slate-400">{user.two_factor_enabled ? "Enabled" : "Disabled"}</td><td className="py-4 pr-4 text-slate-500">{formatDate(user.last_login_at)}</td><td className="py-4 text-right"><button disabled={actionId === user.id} onClick={() => void toggleUser(user)} className="rounded-lg border border-slate-700 px-3 py-1.5 text-xs font-semibold text-slate-300 hover:border-slate-500 disabled:opacity-50">{actionId === user.id ? "Saving..." : user.is_active ? "Suspend" : "Activate"}</button></td></tr>)}</tbody>
          </table>
        </div>
