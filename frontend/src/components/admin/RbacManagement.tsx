...
type CreateForm = { full_name: string; email: string; password: string; role_id: string; reason: string };
...
export default function RbacManagement() {
  ...
  async function createAdmin(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await adminFetch("/api/v1/admin/rbac/admins", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
      const data = (await response.json()) as Admin & { detail?: string };
      if (!response.ok) throw new Error(data.detail || "Unable to create admin");
      setNotice(`Admin ${data.email} created successfully.`); setForm(emptyForm); setShowCreate(false); await load();
    } catch (err) { setError(err instanceof Error ? err.message : "Unable to create admin"); }
    finally { setCreating(false); }
  }

  async function saveRole(admin: Admin) {
    const roleName = selected[admin.id]; const role = roles.find((item) => item.name === roleName); if (!role) return;
    const reason = window.prompt(`Reason for assigning ${role.name}:`); if (!reason || reason.trim().length < 3) return;
    setSaving(admin.id); setError(""); setNotice("");
    try {
      const response = await adminFetch(`/api/v1/admin/rbac/admins/${admin.id}/roles`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ role_ids: [role.id], reason: reason.trim() }) });
      ...
      setAdmins((items) => items.map((item) => item.id === admin.id ? { ...item, roles: data.roles } : item)); setNotice(`Role updated for ${admin.email}.`);
  ...
  return <div className="mx-auto max-w-7xl space-y-6">
    <div className="flex items-start justify-between gap-4"><div><h2 className="text-2xl font-bold tracking-tight">Admin RBAC</h2><p className="mt-2 text-sm text-slate-500">Create staff administrators, assign controlled roles, and review effective permissions.</p></div><button onClick={() => { setShowCreate(true); setError(""); setNotice(""); }} className="rounded-lg bg-cyan-500 px-4 py-2.5 text-sm font-bold text-slate-950">+ Create Admin</button></div>
