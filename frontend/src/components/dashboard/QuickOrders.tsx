...
type QuickOrdersProps = { showBalance?: boolean };
...
export default function QuickOrders({ showBalance = true }: QuickOrdersProps) {
  ...
  useEffect(() => {
    ...
    const refresh = () => {
      void getOpenOrders()
        .then((data) => {
          if (!active) return;
          setOrders(Array.isArray(data) && data.length ? data : fallbackOrders);
        })
        .catch(() => {
          if (active) setOrders(fallbackOrders);
        });
    };
