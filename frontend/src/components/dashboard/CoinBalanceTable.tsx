...
export default function CoinBalanceTable() {
  ...
  const coins: CoinBalance[] = assets.length
    ...
    : [
        { symbol: "BTC", balance: "0.025 BTC", netCost: 250000, value: 300000, change: "+2.45%" },
        { symbol: "ETH", balance: "2.5 ETH", netCost: 200000, value: 250000, change: "+1.20%" },
        { symbol: "USDT", balance: "500 USDT", netCost: 48000, value: 48000, change: "+0.05%" },
      ];
    ...
    <section className="rounded-xl border border-white/10 bg-[#111318] p-6">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {coins.map((coin) => (
              <tr key={coin.symbol} className="border-b border-white/10 text-white hover:bg-white/5">
                <td className="p-3">
                ...
                <td className="p-3 font-semibold">₹ {(coin.value || 0).toLocaleString("en-IN")}</td>
                <td className="p-3 text-green-400">{coin.change}</td>
                <td className="p-3">
                  <div className="flex gap-2">
                    <button className="rounded-md bg-blue-600 px-3 py-1 text-xs">Deposit</button>
                    <button className="rounded-md border border-white/20 px-3 py-1 text-xs">Withdraw</button>
                  </div>
                </td>
              </tr>
            ))}
