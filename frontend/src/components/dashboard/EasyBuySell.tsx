...
export default function EasyBuySell() {
  ...
  return (
    <section className="rounded-2xl border border-gray-800 bg-[#111318] p-6">
      ...
      <div className="grid grid-cols-2 rounded-xl bg-[#0b0e11] p-1 mb-4"><button onClick={() => setMode("BUY")} className={mode === "BUY" ? "bg-green-600 rounded-lg py-3" : "py-3 text-gray-400"}>BUY</button><button onClick={() => setMode("SELL")} className={mode === "SELL" ? "bg-red-600 rounded-lg py-3" : "py-3 text-gray-400"}>SELL</button></div>
      <div className="space-y-4">
        <div className="rounded-xl border border-gray-800 bg-[#0b0e11] p-4"><p className="text-gray-400">You Pay</p><div className="flex justify-between"><input value={amount} onChange={(e)=>setAmount(e.target.value)} className="bg-transparent text-xl text-white outline-none" placeholder="0.00"/><span>{mode === "BUY" ? "INR" : selectedCoin}</span></div><p className="text-xs text-gray-400 mt-2">Available Balance: ₹{availableINR}</p></div>
        <div className="rounded-xl border border-gray-800 bg-[#0b0e11] p-4"><p className="text-gray-400">You Receive</p><div className="flex justify-between text-xl"><span>{receiveAmount}</span><span>{selectedCoin}</span></div></div>
      </div>
      {insufficientBalance && <p className="mt-3 text-red-400 text-sm">Insufficient INR Balance</p>}
      <button disabled={!amount || insufficientBalance} onClick={() => setShowConfirm(true)} className="mt-5 w-full rounded-xl bg-blue-600 py-3 disabled:opacity-50">Review {mode} {selectedCoin}</button>
      <CoinSelectorModal open={showCoinModal} onClose={() => setShowCoinModal(false)} onSelect={setSelectedCoin}/>
      <OrderConfirmationModal open={showConfirm} coin={selectedCoin} mode={mode} payAmount={payAmount} receiveAmount={receiveAmount} fee={fee} onCancel={()=>setShowConfirm(false)} onConfirm={()=>setShowConfirm(false)}/>
    </section>
  );
}
