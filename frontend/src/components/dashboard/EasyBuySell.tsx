...
export default function EasyBuySell() {
  ...
  return (
    <section className="h-full rounded-lg border border-white/[0.06] bg-[#10161d] p-4 text-white">
      ...
      <button disabled={!amount || insufficientBalance || submitting} onClick={() => setShowConfirm(true)} className={`mt-3 w-full rounded-md py-2 text-[10px] font-bold text-white disabled:opacity-40 ${mode === "BUY" ? "bg-gradient-to-r from-emerald-600 to-emerald-500" : "bg-gradient-to-r from-red-600 to-red-500"}`}>{submitting ? "Submitting..." : `Continue ${mode} ${selectedCoin}`}</button>
